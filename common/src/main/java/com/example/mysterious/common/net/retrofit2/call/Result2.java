...
public final class Result2<T> {
  ...
  public static <T> Result2<T> error(Throwable error) {
    if (error == null) throw new NullPointerException("error == null");
    return new Result2<>(null, error);
  }

  @SuppressWarnings("ConstantConditions") // Guarding public API nullability.
  public static <T> Result2<T> response(Response<T> response) {
    if (response == null) throw new NullPointerException("response == null");
    return new Result2<>(response, null);
  }
  ...
  private Result2(@Nullable Response<T> response, @Nullable Throwable error) {
